...
pub async fn create_relation_type(
    ...
    if req.name.trim().is_empty() {
        return Err(InkwellError::Validation(
            "Relation type name cannot be empty".into(),
        ));
    }
    if req.label.trim().is_empty() {
        return Err(InkwellError::Validation(
            "Relation type label cannot be empty".into(),
        ));
    }
    let conn = state
        .db
...
#[tauri::command]
pub async fn get_relation_type(state: State<'_, AppState>, id: String) -> Result<RelationType> {
    let conn = state
        .db
        .lock()
        .map_err(|_| InkwellError::Internal("DB lock poisoned".into()))?;
...
#[tauri::command]
pub async fn delete_relation_type(state: State<'_, AppState>, id: String) -> Result<()> {
    let conn = state
        .db
        .lock()
        .map_err(|_| InkwellError::Internal("DB lock poisoned".into()))?;
