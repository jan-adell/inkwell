  ...
  return (
    <div className="flex items-center gap-0.5 px-4 py-1.5 border-b border-ink-border flex-shrink-0">
      ...
      <div className="ml-auto flex items-center gap-0.5">
        {zoomBtn(onZoomOut, "Zoom out", ZoomOut, canZoomOut)}
        <span className="text-xs text-ivory-ghost font-mono w-9 text-center tabular-nums">
          {Math.round(zoom * 100)}%
        </span>
        {zoomBtn(onZoomIn, "Zoom in", ZoomIn, canZoomIn)}
      </div>
    </div>
  );
}
...
export function DocumentEditor({ documentId, doc }: Props) {
  ...
  function zoomIn() {
    if (!canZoomIn) return;
    const next = ZOOM_LEVELS[zoomIdx + 1];
    setZoom(next);
    saveZoom(next);
  }
  function zoomOut() {
  ...
  const handleChange = useCallback((json: string, text: string) => {
    ...
    debounceTimer.current = setTimeout(() => {
      ...
      if (snap) { pendingSave.current = null; flushSave(currentDocId.current, snap.json, snap.text); }
    }, AUTOSAVE_DELAY_MS);
  }, []);
  ...
  async function handleTitleBlur() {
    ...
    try {
      ...
      updateDocument(updated);
    } catch { setLocalTitle(doc.title); }
  }

  function handleTitleKey(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter") e.currentTarget.blur();
    if (e.key === "Escape") { setLocalTitle(doc.title); e.currentTarget.blur(); }
  }
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Title bar */}
      <div className="flex items-center gap-3 px-6 py-2.5 border-b border-ink-border flex-shrink-0">
        <input
          type="text"
          value={localTitle}
          ...
          className="flex-1 bg-transparent text-sm font-display text-ivory tracking-wide focus:outline-none selectable placeholder:text-ivory-ghost"
          placeholder="Untitled"
        />
        <select
          value={doc.status}
          onChange={handleStatusChange}
          className={["bg-transparent border-none text-xs font-mono focus:outline-none cursor-pointer flex-shrink-0", STATUS_COLORS[doc.status]].join(" ")}
        >
          {(Object.keys(STATUS_LABELS) as Document["status"][]).map((s) => (
            <option key={s} value={s} className="bg-ink-deep text-ivory">{STATUS_LABELS[s]}</option>
          ))}
        </select>
      ...
      {/* Footer */}
      <div className="px-6 py-1.5 border-t border-ink-border flex-shrink-0 flex items-center gap-4">
        <span className="text-xs text-ivory-ghost font-mono">
          {doc.word_count} {doc.word_count === 1 ? "word" : "words"}
        </span>
        <span className="text-xs text-ivory-ghost font-mono">Created {fmtDate(doc.created_at)}</span>
        <span className="text-xs text-ivory-ghost font-mono">Modified {fmtDateTime(doc.updated_at)}</span>
      </div>
    </div>
  );
}
